...
describe("Append-triggered fanout (triggerFanout)", () => {
  it("close-only append does NOT fan out to subscribers", async () => {
    ...
    const subResp = await fetch(`${BASE_URL}/v1/estuary/subscribe/${projectId}/${sourceStreamId}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("unsubscribing prevents future fanout", async () => {
    ...
    const readResp = await fetch(`${BASE_URL}/v1/stream/${estuaryPath}?offset=${ZERO_OFFSET}`);
  ...
  it("append to source with no SUBSCRIPTION_DO binding (no fanout, no error)", async () => {
    ...
    const readResp = await fetch(`${BASE_URL}/v1/stream/${sourceStreamPath}?offset=${ZERO_OFFSET}`);
    expect(readResp.status).toBe(200);
