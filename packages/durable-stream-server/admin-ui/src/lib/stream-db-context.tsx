...
// Use same origin for API calls (relative paths)
const getServerUrl = () => {
...
export function getUserId(): string {
  let userId = localStorage.getItem("durable-streams-admin-user-id");
  if (!userId) {
    userId = `admin-${crypto.randomUUID()}`;
    try {
      localStorage.setItem("durable-streams-admin-user-id", userId);
    } catch {
      // Fallback for privacy mode
      sessionStorage.setItem("durable-streams-admin-user-id", userId);
    }
  }
  return userId;
}
...
export function getSessionId(): string {
  let sessionId = sessionStorage.getItem("durable-streams-admin-session-id");
  if (!sessionId) {
    sessionId = `session-${crypto.randomUUID()}`;
    sessionStorage.setItem("durable-streams-admin-session-id", sessionId);
  }
  return sessionId;
}
        ...
        onMutate: (data: PresenceData) => {
          ...
          if (existing) {
            db.collections.presence.update(data.sessionId, (draft) => {
              for (const key of Object.keys(data) as Array<keyof PresenceData>) {
...
export function StreamDBProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initDBs = async () => {
      ...
      const registryStream = new DurableStream({
        url: `${serverUrl}/v1/stream/__registry__`,
      });
      ...
      const presenceStream = new DurableStream({
        url: `${serverUrl}/v1/stream/__presence__`,
      });
      ...
      registryDB = await createRegistryDB(`${serverUrl}/v1/stream/__registry__`);
      ...
      presenceDB = await createPresenceDB(`${serverUrl}/v1/stream/__presence__`);
  ...
  return <StreamDBContext.Provider value={dbs}>{children}</StreamDBContext.Provider>;
}
