...
export function usePresence() {
  ...
  const params = useParams({ strict: false }) as { streamId?: string };
  ...
  useEffect(() => {
    presenceDB.actions.updatePresence({
      ...
      streamPath: params.streamId,
      isTyping: false,
      lastSeen: Date.now(),
      color: userColor,
      sessionId,
    });
  }, [location.pathname, params.streamId, userId, sessionId, userColor, presenceDB]);
