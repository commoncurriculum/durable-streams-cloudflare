...
type Action = "subscribe" | "unsubscribe" | "publish" | "touch" | "delete";
...
function TestPage() {
  ...
  // publish fields
  const [pubStreamId, setPubStreamId] = useState("");
  const [contentType, setContentType] = useState("application/json");
  const [body, setBody] = useState("");

  // touch / delete fields
  const [touchSessionId, setTouchSessionId] = useState("");
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      switch (action) {
        case "subscribe":
          if (!subSessionId.trim() || !subStreamId.trim()) return;
          payload = {
            action,
            sessionId: subSessionId.trim(),
            streamId: subStreamId.trim(),
          };
          break;
        case "unsubscribe":
          if (!subSessionId.trim() || !subStreamId.trim()) return;
          ...
          break;
        case "publish":
          if (!pubStreamId.trim()) return;
          payload = {
            action,
            streamId: pubStreamId.trim(),
            contentType,
            body,
          };
          break;
        case "touch":
          if (!touchSessionId.trim()) return;
          payload = { action, sessionId: touchSessionId.trim() };
          break;
        case "delete":
          if (!deleteSessionId.trim()) return;
          payload = { action, sessionId: deleteSessionId.trim() };
          break;
      }
      ...
      // Connect SSE for subscribe to see fanout
      if (action === "subscribe" && subSessionId.trim()) {
        setSseUrl(
          `/api/sse/session:${encodeURIComponent(subSessionId.trim())}?live=sse&offset=now`,
        );
      }
    } catch (e) {
  ...
  return (
    <div className="grid min-h-[500px] grid-cols-1 gap-6 lg:grid-cols-[2fr_3fr]">
      ...
      <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5">
        <FormLabel first>Action</FormLabel>
        ...
        {action === "touch" && (
          <>
            <FormLabel>Session ID</FormLabel>
            <TextInput
              value={touchSessionId}
              onChange={setTouchSessionId}
              placeholder="my-session"
            />
          </>
        )}
