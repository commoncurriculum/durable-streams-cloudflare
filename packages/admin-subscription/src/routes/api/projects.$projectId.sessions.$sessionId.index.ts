...
export const Route = createFileRoute("/api/projects/$projectId/sessions/$sessionId/")({
  server: {
      ...
      createHandlers({
        GET: async ({ params }) => {
          ...
          const subscription = (env as Record<string, unknown>).SUBSCRIPTION as
            ...
            | undefined;
          if (!subscription) {
            return new Response(JSON.stringify({ error: "SUBSCRIPTION service not configured" }), {
              status: 500,
