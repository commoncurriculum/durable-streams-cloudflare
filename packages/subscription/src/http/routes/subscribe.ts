...
import { ESTUARY_ID_PATTERN } from "../../constants";
import type { AppEnv } from "../../env";

// #region synced-to-docs:subscribe-schema
const subscribeSchema = type({
  estuaryId: type("string > 0").pipe((s, ctx) => {
    if (!ESTUARY_ID_PATTERN.test(s)) return ctx.error("Invalid estuaryId format");
    return s;
  }),
  "contentType?": "string",
});
...
subscribeRoutes.post("/subscribe/:projectId/:streamId", arktypeValidator("json", subscribeSchema), async (c) => {
...
subscribeRoutes.delete("/subscribe/:projectId/:streamId", arktypeValidator("json", unsubscribeSchema), async (c) => {
  const projectId = c.req.param("projectId");
  const streamId = c.req.param("streamId");
  const { estuaryId } = c.req.valid("json");
  try {
