...
describe("admin-core smoke", () => {
  let proc: ChildProcess;
  let startupError = "";

  beforeAll(async () => {
    // Override inspector port to avoid conflicts with running dev servers
    proc = spawn(
      "pnpm",
      ["exec", "vite", "dev", "--port", String(PORT)],
      {
        ...
        env: {
          ...
          // Use a unique inspector port so tests don't conflict with running dev servers
          CF_INSPECTOR_PORT: "19232",
        },
      },
    );
  ...
  it("GET /projects/$projectId/streams/$streamId renders the child route, not just the search form", async () => {
    ...
    // we'd see "Enter a stream ID to open or create" instead of stream content.
