...
## Read-after-write consistency (from docs)
...
## X-Cache header (from docs)

- [x] 13. Cache hits return `X-Cache: HIT` — COVERED `edge_cache.test.ts` multiple tests (e.g. "caches mid-stream GET reads", "serves identical body from cache hit")
...
## Cache-Control headers (from protocol)
...
- [x] 22. Long-poll 204: `public, max-age=20` — COVERED `edge_cache.test.ts` "long-poll 204 timeout has max-age=20"
...
- [x] 25. Expired TTL stream: `no-store` — COVERED `edge_cache.test.ts` "expired TTL stream returns Cache-Control: no-store" (verifies 404 after expiry; `cacheControlFor` returns `no-store` when remaining <= 0)
...
## Cache body correctness (from docs)
